...
namespace Assets.Scripts
{
	public class GameEngine
	{
		...
		protected Vector2 MousePos = new Vector2 (-1f, -1f);
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
		...
		public virtual bool GeneratePath ()
		{
			...
			int[] triedDirections = new[] { 0, 0, 0, 0 };

			//Start traveling
			while (currentLevel > 0 && FieldSize.y > 1 && FieldSize.x > 1 && GeneratedPath.Any() && Field.Any ()) {
				...
				case (3):
					...
					break;
				default:
					Debug.LogWarning (currentDirection.ToString () + " was picked");
					break;
				}
				...
				if (CheckMove (currentPosition) && currentDirection >= 0) {
					GeneratedPath.Add (Field [new[] {
						Indexes [0] [(int)currentPosition.x],
						Indexes [1] [(int)currentPosition.y]
					}]);
		...
		public bool CheckMove (Vector2 posCheck)
		{
			...
			if ((int)posCheck.x >= FieldSize.x || (int)posCheck.y >= FieldSize.y)
				return false;
			if (Field [new[] {
				Indexes [0] [(int)posCheck.x],
				Indexes [1] [(int)posCheck.y]
			}].Type == 0)
				return false;
			return true;
		}
		...
		public virtual void Update ()
		{
			...
			lock (FieldLock) {
				...
				if (Field.Any () && Indexes.All (ind => ind.Any ())) {
					Vector2 min = Field [new[] {
						Indexes [0].FirstOrDefault (),
						Indexes [1].FirstOrDefault ()
					}].Tesseract.transform.position;
					Vector2 max = Field [new[] {
						Indexes [0].LastOrDefault (),
						Indexes [1].LastOrDefault ()
					}].Tesseract.transform.position;
					...
					if (Math.Abs (MousePos.x - LastPos.x) > 0.0001 || Math.Abs (MousePos.y - LastPos.y) > 0.0001)
						Field [new[] {
							Indexes [0] [(int)MousePos.x],
							Indexes [1] [(int)MousePos.y]
						}].Traveled ();
				}
			...
			if (Input.GetMouseButtonDown (0)) {
				if (FieldSize [0] <= 1 && FieldSize [1] <= 1)
			...
			if (Input.GetKey (KeyCode.Delete) && Started) {
				LevelComplete = true;
			}
		}
		...
		//Cleanup before moving from one mode/level to another, remove field
		public virtual void CleanupAssets ()
		{
			lock (FieldLock) {
				var toRemove = new List<KeyValuePair<Guid[], Crystal>> ();
				foreach (var locatedCrystal in Field) {
					locatedCrystal.Value.DestroyCrystal ();
					toRemove.Add (locatedCrystal);
				}

				toRemove.ForEach (vp => Field.Remove (vp.Key));
			}
				...
				for (var i = 0; i < numb; i++) {
					...
					try {
						...
						for (var j = 0; j < removeItemsCount; j++) {
							...
							if (axis == 0) //
								toRemove = new[] {
									Indexes [0] [index >= 0 ? (index >= Indexes [0].Count () ? Indexes [0].Count () - 1 : index) : Indexes [0].Count - 1],
									Indexes [1] [j]
								};
							else// if(axis == 1)
								toRemove = new[] {
									Indexes [0] [j],
									Indexes [1] [index >= 0 ? (index >= Indexes [1].Count () ? Indexes [1].Count - 1 : index) : Indexes [1].Count - 1]
								};
